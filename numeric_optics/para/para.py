...
class Para:
    ...
    def __matmul__(f, g):
        return Para(lambda: (f.param(), g.param()), f.arrow @ g.arrow)

    def __rshift__(f, g):
        # NOTE: order of parameters is in reverse with respect to order of composition
        return Para(lambda: (g.param(), f.param()), lens.assocL >> (lens.identity @ f.arrow) >> g.arrow)

def to_para(f):
    """ Lift a Lens into a Para using the unit object as its parameter space """
    return Para(lambda: None, lens.snd >> f)
...
def dense(shape, activation, initialize_weights=initialize.normal(0, 0.01)):
    ...
    (a, b) = shape
    p = lambda: (np.zeros(b), initialize_weights((b, a)))
